    ...
    <div className="h-screen flex items-center justify-center bg-pattern bg-no-repeat bg-center">
            ...
            <div className="h-16 bg-zinc-900 px-4 rounded-xl flex items-center shadow-shape gap-3">
              <button type="button" onClick={openGuestsModal} className="flex items-center gap-2 flex-1 text-left">
                ...
                {emailsToInvite.length > 0 ? (
                  <span className="text-zinc-100 text-lg flex-1">{emailsToInvite.length == 1 ? (
                    <p>{emailsToInvite.length} pessoa convidada</p>
                  ) : (
                    <p>{emailsToInvite.length} pessoa(s) convidada(s)</p>
                  )}</span>
                ) : (
      ...
      {isGuestsModalOpen && (
        <div className="fixed indent-0 bg-black/60 h-screen w-full flex items-center justify-center">
          ...
          <div className="w-[640px] min-h-60 rounded-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5"> {/*min-h-60*/}
              ...
              {emailsToInvite.map(email => {
                  return (
                    <div key={email} className="py-1.5 px-2.5 rounded-md bg-zinc-800 flex items-center gap-2">
                      ...
                      <button type="button"><X onClick={() => removeEmailsFromInvite(email)} className="size-4 text-zinc-400"/></button>
                    </div>
                  )
                })
              }
            ...
            <div className="w-full h-px bg-zinc-800">
              <form onSubmit={addNewEmailToInvite} className="p-2.5 bg-zinc-950 h-16 border border-zinc-800 rounded-lg flex items-center gap-2">
                <div className="px-2 flex items-center flex-1 gap-2">
                  <AtSign className="text-zinc-400 size-5" />
                    <input 
                      type="email" 
                      name="email"
                      placeholder="Digite o e-mail do cocvidado" 
      ...
      {isConfirmTripModalOpen && (
        <div className="fixed indent-0 bg-black/60 h-screen w-full flex items-center justify-center">
          <div className="w-[640px] min-h-60 rounded-xl py-5 px-6 shadow-shape bg-zinc-900 space-y-5">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
            ...
            <form onSubmit={addNewEmailToInvite} className="space-y-3">
              ...
              <button type="submit" className="bg-lime-300 text-lime-950 rounded-lg px-5 h-11 font-medium 
              flex items-center gap-2 hover:bg-lime-400 w-full justify-center">
